---
description: "mafia-ace: AI agents playing Mafia with self-improving cheatsheets and Weave observability"
alwaysApply: true
---

Core standards:

- Use Weave for observability; do not build custom logging/tracing
- Clean Code requirements are strictly enforced; we follow DDD principles
...
IMPORTANT: Use `ruff` for Python linting/formatting. Run `uv run ruff check` and `uv run ruff format` before commits.

# Mafia ACE Agent Rules

AGENTS.md (includes symlinks like CLAUDE.md) is the index and enforcement surface. **All hashed rules must be stated succinctly in this file**. Supporting documentation lives under `docs/` by narrow topic (each file <= 350 LOC) and explains **how** and **why** when needed.
...
- [PM1a] Use `uv` for all Python package operations; NO `pip` or `requirements.txt`
- [PM1b] Add packages: `uv add <package>` (updates `pyproject.toml` automatically)
- [PM1c] Source of truth: `pyproject.toml` in each workspace (`backend/`, root)
- [PM1d] Frontend: use `npm` for Node packages in `frontend/`
...
- [CC1d] Clean Architecture: dependencies point inward; domain logic is framework-free
- Contract: `docs/contracts/project-code-requirements.md`

### [ID1] Idiomatic Patterns

- [ID1a] Prefer idiomatic/default platform patterns (Weave ops, FastAPI dependencies)
- [ID1b] Custom patterns require strong justification
- [ID1c] Don't reinvent: use Weave for observability, Pydantic for validation
...
- [FS1d] Domain logic stays framework-free (per [CC1d])
...
- [WV1a] Use `@weave.op()` for all LLM calls and game operations
- [WV1b] Use Weave traces for observability; don't build custom logging
- [WV1c] Use Weave evaluations for measuring cheatsheet helpfulness
- Contract: `docs/contracts/project-code-requirements.md`

### [AR1] Architecture Layers

- [AR1a] Backend canonical roots: `api/`, `db/`, `game/`, `models/`, `websocket/`
- [AR1b] Game logic (`game/`) should not import API/WebSocket concerns
- [AR1c] Models (`models/`) are pure Pydantic; no business logic
- [AR1d] Database layer (`db/`) handles persistence; returns domain models
- [AR1e] API routes are thin wrappers; delegate to game/db layers
- Contract: `docs/contracts/project-code-requirements.md`

### [FE1] Frontend Standards

- [FE1a] SvelteKit with Svelte 5 runes syntax
- [FE1b] TypeScript strict mode; no `any` types
- [FE1c] Components in `lib/components/`; types in `lib/types.ts`
- [FE1d] API client in `lib/api.ts`

### [TS1] Testing Standards

- [TS1a] Add/maintain tests for new functionality
- [TS1b] Assert observable behavior, not internals
- [TS1c] Use Weave evaluation for cheatsheet quality testing
- [TS1d] Naming: `test_{module}.py` with `test_{function}` functions

### [VR1] Verification Gates

- [VR1a] Run verification before any commit
- [VR1b] Backend: `uv run ruff check && uv run ruff format --check`
- [VR1c] Frontend: `npm run check` (svelte-check + TypeScript)
- Contract: `docs/contracts/project-code-requirements.md`
